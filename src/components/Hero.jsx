...
const Hero = () => {
  ...
  useGSAP(() => {
    ...
    // Scroll range for video: different trigger points on mobile vs desktop
    const startValue = isMobile ? 'top 50%' : 'center 60%';   // When timeline starts (trigger position vs viewport)
    const endValue = isMobile ? '120% top' : 'bottom top';    // When timeline ends (longer range on mobile)

    // Scroll-driven video: playback (currentTime) is tied to scroll progress
    const video = videoRef.current;
    if (video) {
      let tl = gsap.timeline({
        scrollTrigger: {
          trigger: "video",      // Element that defines the scroll range (the <video> or its wrapper)
          start: startValue,     // Timeline starts when trigger hits this viewport position
          end: endValue,        // Timeline ends when trigger reaches this viewport position
          scrub: true,          // Progress follows scroll (no auto-play); scroll back = video rewinds
          pin: true,            // Pin the trigger element in place while the timeline is active
        },
      });
